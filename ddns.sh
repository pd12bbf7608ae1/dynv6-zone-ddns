...
if [ "$empty_flag" = "1" ]; then
    echo "zone $zone_ipv4 $zone_ipv6" >> "$history_file"
    if [ -n "$zone_ipv4" ]; then
        zone_ipv4_update
        if [ "$?" -ne "0" ]; then
    ...
    test "$update_flag" = "1" && sed -i "s/^zone.*/zone $zone_ipv4 $zone_ipv6/g" $history_file
...
do
    ...
    if [ "$empty_flag" = "1" ]; then
        echo "host#$i $host_ipv4 $host_ipv6" >> "$history_file"
        if [ -n "$host_ipv4" ]; then
